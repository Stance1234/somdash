...
function UserWithdrawal({ data }) {
  ...
  function handleChange(id) {
    const selectedBalance = data.find(item => item._id === id)
    if (selectedBalance) {
      setType(selectedBalance.type)
      setAct(id)
    }
  }
              ...
              {data && data.length > 0 ? (
                data.map((item) => (
                  <option key={item._id} value={item._id}>
                    {item.name} ({item.type === "trading" ? "Trading" : "Mining"}) - $ {commaNumber(item.balance.toFixed(2))}
                  </option>
                ))
              ) : (
...
export async function getServerSideProps(context) {
  try {
    ...
    // Get trading balance
    const tradeBalance = await UserTradingFundModel.findOne({ _userId: user._id })
    
    // Get mining balances
    const miningBalances = await UserMiningContractModel.find({ _userId: user._id })
      .populate({
        path: '_contractId',
        model: MiningContractModel,
        populate: {
          path: '_assetId',
          model: AssetModel
        }
    ...
    // Add trading balance if exists
    if (tradeBalance) {
      data.push({
        _id: tradeBalance._id.toString(),
    ...
    // Add mining balances if exist
    if (miningBalances?.length > 0) {
      miningBalances.forEach(balance => {
        if (balance._contractId && balance._contractId._assetId) {
          data.push({
            _id: balance._id.toString(),
            name: balance._contractId.name || "Mining Contract",
            balance: balance.quantity * balance._contractId._assetId.price,
            type: "mining"
          })
        }
      })
    ...
    return {
      props: {
        data: JSON.parse(JSON.stringify(data)) // Ensure proper serialization
      },
    }
  } catch (error) {

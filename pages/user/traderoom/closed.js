...
function OpenTradeList({ openTrades }) {
    ...
    return (
        <AuthUserLayout>
            <Head>
                <title>Closed Trades - Trademark</title>
            </Head>
            <div className="container">
                <div className="row">
                    <div className="col">
                        <h2 style={{ textAlign: "center" }}>Closed Trades</h2>
                    </div>
                </div>
                <div className="row">
...
export async function getServerSideProps(context) {
    ...
    try {
        ...
        const openTradesDB = await UserTradingContractModel.find({ _userId: user.id, status: "close" })

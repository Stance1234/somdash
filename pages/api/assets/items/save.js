...
async function handler(req, res) {
    ...
    if (req.method === 'POST') {
        ...
        try {
            ...
            const assets = await AssetModel.findById(assetId)

            if(!assets) {
                return res.status(404).json({ status: false, message: "Assets not found"})
            }

            let savedAsset = await SavedAssetModel.findOne({ _userId: user.id, _assetId: assetId })

            if(savedAsset) {
                return res.status(401).json({ status: false, message: "Assets already saved"}) 
            }
            ...
            res.status(200).json({ status: true, message: "Asset saved" })
        } catch(error) {

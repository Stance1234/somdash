  ...
  if (req.method === "POST") {
    try {
      ...
      if (status === true) {
        let htmlData = await getTemplate(req, "deposit", "proof.html")
        htmlData = htmlData.replaceAll("[name]", user.firstname);
        htmlData = htmlData.replace(/\[brand\s+name\]/g, "Growthpoint Invest Ltd"); // Change "Growthpoint Invest Ltd" to "Growthpoint Invest Ltd"

        await transporter.sendMail({
          from: "Growthpoint Invest Ltd", // Change "Growthpoint Invest Ltd" to "Growthpoint Invest Ltd"
          to: email,

...
function PaymentMethodDataItem({ item, count, length }) {
    const [isDeleting, setIsDeleting] = useState(false)
    const { name, address, method } = item
    async function handleDelete({ id }) {
        try {
            setIsDeleting(true)

            const request = await fetch("/api/payment-method/delete", {
                method: "DELETE",
    ...
    return (
        <tr className={`${ count !== length && "border-bottom" }`}>
            ...
            <td className="text-muted fs-15 font-weight-semibold">{ method }</td>
            <td className="text-muted fs-15 font-weight-semibold">{ address }</td>
            <td className="">
...
export default PaymentMethodDataItem